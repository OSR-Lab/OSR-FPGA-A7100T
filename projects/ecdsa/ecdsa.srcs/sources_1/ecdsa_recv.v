`timescale 1ns / 1ps
// Module: ecdsa_recv
// Description: ECDSA top-level UART interface module.
...
//              Performs ECDSA signing and transmits 64 bytes:
...
module ecdsa_recv (
    ...
    input  wire rst,
    output wire busy,
    output reg  ecdsa_trigger,
    output reg  wr_en,
    output wire sign_only_trigger
);
...
// ECDSA signing interface signals
wire ecdsa_busy;
wire ecdsa_done;
...
assign busy = ecdsa_busy | (state != STATE_IDLE);

// Signing-only trigger: high during STATE_SIGN (for power trace capture)
assign sign_only_trigger = (state == STATE_SIGN);
