...
// Tool versions       : 13.4
// Description         : AES-128 ECB mode encryption/decryption core module
//                       - Supports both encryption (enc_dec=0) and decryption (enc_dec=1)
//                       - Implemented with lookup tables (S-Box), compliant with FIPS-197
//                       - Three-part state machine: IDLE -> KEY_EXP (10-round key expansion) -> ROUND_LOOP (10 rounds of encryption/decryption)
//                       - Uses combinational S-Box lookups, completing one round per clock cycle
//
// Dependencies        : subbytes, invsubbytes, subword, mixcolumns, inv_mixcolumn
//
// Version             : 1.0
...
module aes128(
  ...
  input               enc_dec,      // encryption/decryption select: 0 = encrypt, 1 = decrypt
  input               key_exp,      // key expansion start
  input               start,        // encryption or decryption start
  output reg          key_val,      // key expansion done flag (round keys valid)
  output reg          text_val,     // output data valid flag (high for one cycle when encryption/decryption completes)
  input      [127:0]  key_in,       // 128-bit initial key input
  input      [127:0]  text_in,      // 128-bit plaintext (encryption) or ciphertext (decryption) input
  output     [127:0]  text_out,     // 128-bit ciphertext (encryption) or plaintext (decryption) output
  output reg          busy          // busy signal, high while key expansion or encryption/decryption is in progress
);

// State machine state definitions
// IDLE:       idle, waiting for a key expansion or encryption/decryption start
// KEY_EXP:    key expansion phase, iteratively derives the 10 round keys from the initial key
// ROUND_LOOP: encryption/decryption phase, 10 rounds in total
`define  IDLE        3'h0           // idle state
`define  KEY_EXP     3'h1           // key expansion state
`define  ROUND_LOOP  3'h2           // encryption/decryption state

// ==================================================================
// Internal signal declarations
// ==================================================================
  reg       [2:0]  now_state;       // current state register
  reg       [2:0]  next_state;      // next state (combinational)
  reg              start_flag;      // pending start flag: if the keys are not ready, run key expansion first, then start encryption/decryption automatically
  reg       [3:0]  round_n;         // round counter: 0-10, tracks the current round

// Encryption key expansion signals
  reg      [31:0]  w[0:3];          // current 128-bit round key, split into four 32-bit words w[0]-w[3]
  wire     [31:0]  rotword;         // RotWord: rotate w[3] left by one byte (FIPS-197 Section 5.2)
  wire      [7:0]  rcon;            // round constant Rcon: powers of x in GF(2^8), i.e. {01,02,04,08,10,20,40,80,1b,36}
  wire     [31:0]  temp;            // SubWord output: S-Box byte substitution of the RotWord result
  wire    [127:0]  next_key;        // next round key: derived from the current round key
  reg     [127:0]  round10_key;     // round 10 key latch: decryption expands backwards starting from the round 10 key

// Decryption (inverse) key expansion signals
// decryption walks the round keys backwards from round 10 down to round 0
  reg      [31:0]  iw[0:3];         // working register for the current decryption round key
  wire      [7:0]  ircon;           // inverse round constant (reverse order compared to encryption)
  wire     [31:0]  itemp;           // iw[3]^iw[2], used to recover the previous round key
  wire     [31:0]  sword;           // SubWord substitution of itemp
  wire     [31:0]  irotword;        // RotWord rotation of sword
  wire    [127:0]  next_ikey;       // next key in the reverse direction (actually the previous round key)

// Encryption data path signals
  reg      [31:0]  state[0:3];      // encryption state registers, 4x32 bits = 128-bit state matrix
  wire      [7:0]  s_box[0:3][0:3]; // SubBytes output (4x4 bytes)
  wire      [7:0]  s_row[0:3][0:3]; // ShiftRows output (row shift)
  wire      [7:0]  m_col[0:3][0:3]; // MixColumns output (column mixing)
  wire    [127:0]  add_roundkey0;   // round 0 AddRoundKey: plaintext XOR initial key
  wire    [127:0]  add_roundkey;    // rounds 1-9 AddRoundKey: MixColumns result XOR round key
  wire    [127:0]  cipher_text;     // round 10 output: ShiftRows result XOR round key (no MixColumns)

// Decryption data path signals
  reg      [31:0]  istate[0:3];     // decryption state registers, 4x32 bits = 128-bit state matrix
  wire      [7:0]  is_row_in[0:3][0:3]; // InvShiftRows input (state matrix split into bytes)
  wire      [7:0]  is_row[0:3][0:3];    // InvShiftRows output (inverse row shift)
  wire      [7:0]  is_box[0:3][0:3];    // InvSubBytes output (inverse S-Box substitution)
  wire     [31:0]  im_col[0:3];         // InvMixColumns output (inverse column mixing)
  wire    [127:0]  add_iroundkey0;  // decryption round 0: ciphertext XOR round 10 key
  wire    [127:0]  add_iroundkey;   // decryption rounds 1-10: InvSubBytes result XOR reverse-ordered round key
  wire    [127:0]  plainte_text;    // final decryption output (plaintext)

// ================================================================================
// Logic implementation
// ================================================================================
// --------------------------------------------------------------------------------
// Main state machine (combinational: the next state follows the current state and the inputs)
// State transitions:
//   IDLE -> KEY_EXP:    key_exp asserted, or start asserted while the keys are not ready (key_val=0)
//   IDLE -> ROUND_LOOP: start asserted with the keys already ready, or start_flag pending
//   KEY_EXP -> IDLE:    key expansion finished (round_n=10)
//   ROUND_LOOP -> IDLE: encryption/decryption finished (round_n=10)
// --------------------------------------------------------------------------------
  always @( now_state or enc_dec or key_exp or start or start_flag or round_n or key_val ) begin
    case ( now_state )
      `IDLE       : if ( key_exp == 1'b1 ) next_state = `KEY_EXP;       // idle: key expansion requested
                    else if ( start == 1'b1 )
                      if ( key_val == 1'b0 ) next_state = `KEY_EXP;    // keys not ready, expand them first
                      else next_state = `ROUND_LOOP;                   // keys already ready, start encryption/decryption directly
                    else if ( start_flag == 1'b1 ) next_state = `ROUND_LOOP; // start encryption/decryption automatically after key expansion
                    else next_state = `IDLE;
      `KEY_EXP    : if ( round_n == 4'd10 ) next_state = `IDLE;         // key expansion: return to idle after 10 rounds
                    else next_state = `KEY_EXP;
      `ROUND_LOOP : if ( round_n == 4'd10 ) next_state = `IDLE;         // encryption/decryption: return to idle after 10 rounds
                    else next_state = `ROUND_LOOP;
       default    : next_state = `IDLE;
    endcase
  end

  // state register: asynchronous reset, updated on the rising edge
...
// Includes: busy (busy flag), start_flag (pending start), round_n (round counter),
//       key_val (keys valid), text_val (output data valid)
// ------------------------------------------------------------------------------
  always @(posedge clock or negedge resetn) begin
    if ( resetn == 1'b0 ) begin
      ...
      // busy flag: set when key_exp or start is received, cleared when key expansion or encryption/decryption completes
      ...
      // pending start flag: set on start, cleared upon entering ROUND_LOOP
      // purpose: if the keys are not ready, finish key expansion first and then start encryption/decryption automatically
      if ( start == 1'b1 ) start_flag <= 1'b1;
      else if ( now_state == `ROUND_LOOP ) start_flag <= 1'b0;
      else start_flag <= start_flag;

      // round counter: cleared when the state returns to IDLE, otherwise incremented every cycle (0-10)
      if ( next_state == `IDLE ) round_n <= 4'h0;
      else round_n <= round_n + 1'b1;

      // keys valid flag: cleared on key_exp, set when key expansion completes (10 rounds)
      if ( key_exp == 1'b1 ) key_val <= 1'b0;
      else if (( now_state == `KEY_EXP ) && ( round_n == 4'd10 )) key_val <= 1'b1;
      else key_val <= key_val;

      // output data valid flag: held high for one cycle when encryption/decryption completes (round 10)
      if (( now_state == `ROUND_LOOP ) && ( round_n == 4'd10 )) text_val <= 1'b1;
      else text_val <= 1'b0;
    end
  end

  // output selection: encryption outputs state, decryption outputs istate
  assign text_out = ( enc_dec == 1'b0 )? { state[0], state[1], state[2], state[3] } : { istate[0], istate[1], istate[2], istate[3] };


// ----------------------------------------------------------------
// 128-bit encryption key expansion (FIPS-197 Section 5.2)
// iteratively derives the 10 round keys from the initial key, producing a new 128-bit key each round
// algorithm: w'[0] = w[0] ^ SubWord(RotWord(w[3])) ^ Rcon
//       w'[i] = w[i] ^ w'[i-1]  (i=1,2,3)
// ----------------------------------------------------------------
  always @(posedge clock or negedge resetn) begin
    if ( resetn == 1'b0 ) begin
      ...
      // Encryption round key update:
      // the initial key is loaded in IDLE; the next round key is derived iteratively in KEY_EXP/ROUND_LOOP
      ...
      // latch the round 10 key: decryption expands backwards starting from it
      if (( now_state == `KEY_EXP ) && ( round_n == 4'd10 )) round10_key <= { w[0], w[1], w[2], w[3] };
      else round10_key <= round10_key;
    end
  end

  // RotWord: rotate the four bytes of w[3] left by one position
  // example: [a0,a1,a2,a3] -> [a1,a2,a3,a0]
  assign rotword = {w[3][23:0], w[3][31:24]};
  // SubWord: S-Box lookup on every byte of the RotWord result
  subword SubWord ( .a( rotword ), .b( temp ));

  // compute the next round key (FIPS-197 Figure 11)
  // w'[0] = w[0] XOR (SubWord(RotWord(w[3])) XOR Rcon)
  // w'[1] = w[1] XOR w'[0]
  // w'[2] = w[2] XOR w'[1]
  // w'[3] = w[3] XOR w'[2]
  ...
  // round constant Rcon[i]: x^i in GF(2^8) (irreducible polynomial x^8+x^4+x^3+x+1)
  // rounds 0-9 map to {01,02,04,08,10,20,40,80,1b,36}
  // note: 0x1b = x^8 mod (x^8+x^4+x^3+x+1), 0x36 = x^9 mod ...
...
// ----------------------------------------------------------------
// 128-bit inverse key expansion (for decryption)
// decryption uses the round keys in the opposite order: from round 10 back down to round 0
// the previous round key is recovered with the inverse of the encryption key expansion
// ----------------------------------------------------------------
  // inverse round key working registers
  always @(posedge clock or negedge resetn) begin
    if ( resetn == 1'b0 ) begin
      { iw[0], iw[1], iw[2], iw[3] } <= {128{1'b0}};
    end
    else begin
      // when key expansion completes, the final round 10 key is loaded into the inverse key registers
      if (( now_state == `KEY_EXP ) && ( round_n == 4'd10 )) { iw[0], iw[1], iw[2], iw[3] } <= { w[0], w[1], w[2], w[3] };
      // in IDLE the saved round 10 key is reloaded (ready for the next decryption)
      else if ( next_state == `IDLE ) { iw[0], iw[1], iw[2], iw[3] } <= round10_key;
      // during decryption the keys are recovered round by round
  ...
  // inverse key expansion: recover the previous round key by inverting w[i] = w'[i] XOR w'[i-1]
  // itemp = iw[3] XOR iw[2] recovers w[3] of the encryption expansion (previous round)
  assign itemp = iw[3] ^ iw[2];

  // SubWord substitution of the recovered value (same S-Box as the encryption key expansion)
  subword iSubWord ( .a( itemp ), .b( sword ));

  // RotWord: rotate left by one byte
  assign irotword = {sword[23:0], sword[31:24]};

  // compute the next key in the reverse direction (the previous round key of the encryption sequence)
  ...
  // inverse round constant: reverse order compared to encryption (round9 down to round0 map to 01,02,...,36)
...
// ------------------------------------------------------------------------------------------
// ECB-AES128 encryption (Cipher)
// FIPS-197 Section 5.1: encryption flow
//   Round 0:    AddRoundKey (plaintext XOR initial key)
//   Rounds 1-9: SubBytes -> ShiftRows -> MixColumns -> AddRoundKey
//   Round 10:   SubBytes -> ShiftRows -> AddRoundKey (no MixColumns in the last round)
// ------------------------------------------------------------------------------------------
  // encryption state registers: hold the intermediate result of each round
  always @(posedge clock or negedge resetn) begin
    if ( resetn == 1'b0 ) begin
      ...
      if ( enc_dec == 1'b0 ) begin
        if ((( start == 1'b1 ) || ( start_flag == 1'b1 )) && ( round_n == 4'h0 )) begin                                                        // round 0: AddRoundKey only (plaintext XOR initial key)
          { state[0], state[1], state[2], state[3]} <= add_roundkey0;
        end
        else if (( now_state == `ROUND_LOOP ) && ( round_n >= 4'd1 ) && ( round_n <= 4'h9 )) begin   // rounds 1-9: SubBytes->ShiftRows->MixColumns->AddRoundKey
          { state[0], state[1], state[2], state[3]} <= add_roundkey;
        end
        else if (( now_state == `ROUND_LOOP ) && ( round_n == 4'd10 )) begin   // round 10: SubBytes->ShiftRows->AddRoundKey (no MixColumns)
  ...
  // ------------------------------------------------------------------------------------------------------------
  // round 0: AddRoundKey (FIPS-197 Section 5.1.4)
  // XOR the 128-bit plaintext with the initial key (round 0 key)
  // ------------------------------------------------------------------------------------------------------------
  assign add_roundkey0 = text_in ^ { w[0], w[1], w[2], w[3] };

  // ------------------------------------------------------------------------------------------------------------
  // SubBytes transformation (FIPS-197 Section 5.1.1)
  // applies the nonlinear S-Box substitution to every byte of the 4x4 state matrix
  // the S-Box is built from inversion in GF(2^8) plus an affine transform
  // ------------------------------------------------------------------------------------------------------------
  subbytes SubBytes (
  ...
  // rotates each row of the state matrix to the left:
  //   row 0: no shift
  ...
  // multiplies each column of the state matrix in GF(2^8)
  // fixed matrix: [{02,03,01,01},{01,02,03,01},{01,01,02,03},{03,01,01,02}]
  // the four columns instantiate four mixcolumns modules
  // ------------------------------------------------------------------------------------------------------------
  ...
  // XOR the MixColumns output with the current round key
  // ------------------------------------------------------------------------------------------------------------
  // rounds 1-9: MixColumns result XOR round key
  ...
  // round 10: ShiftRows result XOR round key directly (MixColumns skipped)
...
// ------------------------------------------------------------------------------------------------------------
// ECB-AES128 decryption (Inverse Cipher)
// FIPS-197 Section 5.3: decryption flow (equivalent inverse cipher)
//   Round 0:    AddRoundKey (ciphertext XOR round 10 key)
//   Rounds 1-9: InvShiftRows -> InvSubBytes -> AddRoundKey -> InvMixColumns
//   Round 10:   InvShiftRows -> InvSubBytes -> AddRoundKey (no InvMixColumns in the last round)
// ------------------------------------------------------------------------------------------------------------
  // decryption state registers: hold the intermediate result of each round
  always @(posedge clock or negedge resetn) begin
    if ( resetn == 1'b0 ) begin
      ...
      if ( enc_dec == 1'b1 ) begin
        if ((( start == 1'b1 ) || ( start_flag == 1'b1 )) && ( round_n == 4'h0 )) begin      // round 0: AddRoundKey (ciphertext XOR round 10 key)
          { istate[0], istate[1], istate[2], istate[3]} <= add_iroundkey0;
        end
        else if (( now_state == `ROUND_LOOP ) && ( round_n >= 4'd1 ) && ( round_n <= 4'd9 )) begin   // rounds 1-9: InvShiftRows->InvSubBytes->AddRoundKey->InvMixColumns
          { istate[0], istate[1], istate[2], istate[3]} <= { im_col[0], im_col[1], im_col[2], im_col[3] };
        end
        else if (( now_state == `ROUND_LOOP ) && ( round_n == 4'd10 )) begin                         // round 10: InvShiftRows->InvSubBytes->AddRoundKey (no InvMixColumns)
  ...
  // ------------------------------------------------------------------------------------------------------------
  // decryption round 0: AddRoundKey
  // XOR the ciphertext with the round 10 key (the initial round key for decryption)
  // ------------------------------------------------------------------------------------------------------------
  assign add_iroundkey0 = text_in ^ { iw[0], iw[1], iw[2], iw[3] };

  // ------------------------------------------------------------------------------------------------------------
  // InvShiftRows transformation (FIPS-197 Section 5.3.1)
  // rotates each row of the state matrix to the right (opposite direction to ShiftRows):
  //   row 0: no shift
  //   row 1: right by 1 byte (equivalent to left by 3 bytes)
  //   row 2: right by 2 bytes
  //   row 3: right by 3 bytes (equivalent to left by 1 byte)
  // ------------------------------------------------------------------------------------------------------------
  // split the 32-bit state words into four bytes to simplify the row shift
  ...
  // ------------------------------------------------------------------------------------------------------------
  // InvSubBytes transformation (FIPS-197 Section 5.3.2)
  // applies the inverse S-Box lookup to every byte of the InvShiftRows output
  // the inverse S-Box is the inverse mapping of the forward S-Box
  // ------------------------------------------------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------------------------------------------------
  // decryption AddRoundKey: InvSubBytes result XOR reverse-ordered round key
  // shared by rounds 1-10; rounds 1-9 continue with InvMixColumns, round 10 is output directly
  // ------------------------------------------------------------------------------------------------------------
  // rounds 1-10
  ...
  // ------------------------------------------------------------------------------------------------------------
  // InvMixColumns transformation (FIPS-197 Section 5.3.3)
  // multiplies each column of the AddRoundKey result by the inverse matrix in GF(2^8)
  // inverse matrix: [{0e,0b,0d,09},{09,0e,0b,0d},{0d,09,0e,0b},{0b,0d,09,0e}]
  // used in rounds 1-9 only; round 10 skips InvMixColumns
  // the four columns instantiate four inv_mixcolumn modules
  // ------------------------------------------------------------------------------------------------------------
...
// AES forward S-Box, nonlinear substitution of every byte of the 4x4 state matrix (FIPS-197 Section 5.1.1)
// takes 16 input bytes (aij) and outputs 16 substituted bytes (bij) through the 256-entry S-Box table
// the S-Box combines inversion in GF(2^8) with an affine transform to provide nonlinear security
/////////////////////////////////////////////////////////////////////////////////////////////////////////
module subbytes (
...
// AES inverse S-Box, byte substitution used during decryption (FIPS-197 Section 5.3.2)
// the inverse S-Box inverts the forward S-Box, i.e. InvSubBytes(SubBytes(x)) = x
/////////////////////////////////////////////////////////////////////////////////////////////////////////
module invsubbytes (
  input     [7:0]  a00, a10, a20, a30,    // Data input.
  input     [7:0]  a01, a11, a21, a31,    // Data input.
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
// Module Name         : subword
// S-Box substitution of every byte of a 32-bit word, used by key expansion (FIPS-197 Section 5.2)
// splits the input word a[31:0] into four bytes, substitutes each through the S-Box, and outputs b[31:0]
/////////////////////////////////////////////////////////////////////////////////////////////////////////
module subword (
  input  [31:0] a,
  output [31:0] b
...
// column mixing, matrix multiplication in GF(2^8) (FIPS-197 Section 5.1.3)
// fixed matrix coefficients: {02, 03, 01, 01} (cyclically rotated)
...
// m0c = {02}*s0c ^ {03}*s1c ^ {01}*s2c ^ {01}*s3c
// m1c = {01}*s0c ^ {02}*s1c ^ {03}*s2c ^ {01}*s3c
// m2c = {01}*s0c ^ {01}*s1c ^ {02}*s2c ^ {03}*s3c
// m3c = {03}*s0c ^ {01}*s1c ^ {01}*s2c ^ {02}*s3c
// ==============================================
  ...
  // GF(2^8) multiplier mul4x8
  // multiplies the 4-bit multiplier mx by the 8-bit multiplicand sc in GF(2^8)
  // method: multiply sc by each bit of mx with shifts, accumulate, then reduce modulo the irreducible polynomial
  // carry reduction: results wider than 8 bits are reduced with 0x1B (x^8 mod m(x))
  //   bit8 overflow: XOR 0x1B;  bit9 overflow: XOR 0x36(0x1B<<1);  bit10 overflow: XOR 0x6C(0x1B<<2)
  function [7:0] mul4x8;
  input    [3:0] mx;
  input    [7:0] sc;
...
// inverse column mixing with the inverse matrix {0e, 0b, 0d, 09} (FIPS-197 Section 5.3.3)
// the inverse matrix times the forward matrix is the identity matrix, which keeps decryption correct
// multiplication is also done in GF(2^8), reduced modulo the irreducible polynomial 0x11B
/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// ==============================================
// inverse column mixing operations
// m0c = {0e}*s0c ^ {0b}*s1c ^ {0d}*s2c ^ {09}*s3c
// m1c = {09}*s0c ^ {0e}*s1c ^ {0b}*s2c ^ {0d}*s3c
// m2c = {0d}*s0c ^ {09}*s1c ^ {0e}*s2c ^ {0b}*s3c
// m3c = {0b}*s0c ^ {0d}*s1c ^ {09}*s2c ^ {0e}*s3c
// ==============================================
  ...
  // GF(2^8) multiplier (same as mul4x8 in mixcolumns)
  function [7:0] mul4x8;
  input    [3:0] mx;
  input    [7:0] sc;
  reg      [7:0] sxm0, sxm1, sxm2, sxm3;
